...
int   uniform_integer(int a, int b, int stream);
float erlang(int m, float mean, int stream);
int   zipf(float alpha, int n, int stream);
...
void deinit_simlib()
{
    ...
    for (list = 0; list <= maxlist; ++list) {
        ...
        for (it = head [list]; it != NULL; it = sr)
        {
            sr = it->sr;
            free(it->value);
            free(it);
...
int uniform_integer(int a, int b, int stream) /* Uniform variate generation
                                               function. */
{
    return floor(uniform(a, b + 1, stream));
}
...
struct Zipf_distrib
{
    ...
    int                     n;
    float*                  prob;
    struct Zipf_distrib*    next;
};

typedef struct Zipf_distrib Zipf_distrib;

int zipf(float alpha, int n, int stream)
{
    static Zipf_distrib*  collects  = NULL;
    Zipf_distrib*         it        = NULL;
    Zipf_distrib*         distrib   = NULL;
    float*                prob      = NULL;
    float                 p;
    int                   z;
    ...
    if (prob == NULL) {
        ...
        for (int i = 1; i <= n; i++)
            prob[i] = prob[i-1] + pow(i, -alpha) / s;

        distrib = (Zipf_distrib*) malloc(sizeof(Zipf_distrib));
        distrib->alpha = alpha;
        distrib->n     = n;
        distrib->prob  = prob;
        distrib->next  = NULL;
        ...
        if (collects == NULL)
            collects = distrib;
        else it->next = distrib;
    }

    do {
        p = lcgrand(stream);

        for (z = 1; prob[z] < p && z <= n; z++)
            ;
    } while (!(z >= 1 && z <= n));
